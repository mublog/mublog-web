...
export default function createMatcher(path: string): RegExp {
  if (path.startsWith("/")) throw new Error("RoutePath cannot start with an '/'")
  if (path.endsWith("/")) throw new Error("RoutePath cannot end with an '/'")
  if (path === "**") return R_WILDCARD
